...
Para a discretização da equação do calor introduzir-se-á uma malha espacial dada pelos pontos $x_{i} = i\Delta x$, $i=0, \cdots ,N$, com $\Delta x=\frac{1}{N}$. Para a discretização temporal definimos $\Delta t =\frac{T}{M}$, e calculamos aproximações nos instantes $t_{k}=k\Delta t$, $k=1, \cdots,M$. Denotaremos a aproximação para a solução nos pontos de malha $u(t_{k},x_{i})$ por $u_{i}^k$. 
...
Para os pontos interiores a evolução é aproximada pela fórmula de diferenças finitas:
...
No item $a$, primeiro utilizamos a fonte $f(t, x) = 10x^2({x-1}) -60xt + 20t$ com condições de contorno e fronteira nulas, isto é, $u(0,x) = u(t,0) = u(t,1) = 0$. Para estas condições constatamos a solução exata como $u(t, x) = 10tx^{2}({x-1})$ e calculamos as normas dos erros dadas pela fórmula (7) para o instante $t=1$. 
...
A tabela 1 apresenta os resultados das normas dos erros em algumas das simulações.
...
Vamos verificar que o erro de truncamento $\tau$ (que neste caso é igual à norma do erro) é zero. O erro de truncamento é dado por:
...
Portanto, em tempo de execução, o usuário só determina o parâmetro $N=\lambda$. Já o parâmetro $M$ é determinado por $M=NT$. Para efeitos de testes iremos utilizar $T=1$ $\Rightarrow$ $M=N$, porém o programa foi configurado para receber qualquer valor positivo $T$.
